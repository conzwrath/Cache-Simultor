...
cache_t *make_cache(int capacity, int block_size, int assoc) {
  ...
  cache->cache_tags = (unsigned long **)make_2d_matrix(assoc, cache->n_set, sizeof(unsigned long));
  cache->dirty_bits = (bool **)make_2d_matrix(assoc, cache->n_set, sizeof(bool));
...
unsigned long get_cache_tag(cache_t *cache, unsigned long addr) {
  // FIX THIS CODE!
  unsigned int andselector = 0b11111111111111111111111111111111;
...
/* this function takes a chache, an address, and a flag as to whether the 
...
bool access_cache(cache_t *cache, unsigned long addr, bool is_load) {
  ...
  bool cachehit;
  if (is_load == true) {
    if (cache->cache_tags[0][get_cache_index(cache, addr)] == get_cache_tag(cache, addr)) {
      cachehit = true;
    }
    else {
      cachehit = false;
      cache->cache_tags[0][get_cache_index(cache, addr)] = get_cache_tag(cache, addr);
    }
  }
  ...
  return cachehit;  // cache hit should return true
}
